...
{
	// Default value of vector is already 0.
	proteinCount = 0;
	interactionsCount = 0;
}
...
template<typename T>
vector<Protein> ProteinNetwork<T>::getProteins()
{
	return protein;
}
...
{
	if (interactionsCount != 0 && proteinCount != 0) {
...
template<typename T>
ProteinNetwork<Protein>* ProteinNetwork<T>::importNetwork()
{
	int protein_size, interaction_strength;
	string protein_name;
	ProteinNetwork<Protein>* network = new ProteinNetwork<Protein>;
	cout << "How many proteins do you want to enter to the network? " << endl;
	cin >> protein_size;
	Protein* protein = new Protein[protein_size];
	for (int i = 0; i < protein_size; i++) {
		cout << "Enter protein " << i + 1 << " name: " << endl;
		cin >> protein_name;
		Protein p(protein_name);
		network->AddProtein(p);
		protein[i] = p;
	}
	for (int i = 0; i < protein_size; i++) {
		for (int j = i + 1; j < protein_size; j++) {
			cout << endl << "Enter interaction between " << protein[i] << " and " << protein[j] << endl;
			cin >> interaction_strength;
			network->AddInteraction(protein[i], protein[j], interaction_strength);
		}
	}
